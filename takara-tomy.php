<?php
  ...
  $sql = "SELECT * FROM produktai WHERE featured = 8";
  $sql2 = "SELECT * FROM brand WHERE id = 3";
...
<!-- Drop down meniu -->
  ...
  <div class="col-md-8" style="padding-top: 30px;">
   ...
   <?php while($product = mysqli_fetch_assoc($featured)) : ?>
        ...
        <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#details-<?=$product['details'];?>">Read more...</button>
		        <?php
        $query = $db->query("SELECT id FROM produktai where id=1");
        if($query->num_rows > 0){
  ...
  <?php include 'pop-tt-dead-phoenix.php';
        include 'pop-tt-roktavor-heavy.php';
        include 'pop-tt-Valtryek.php';
